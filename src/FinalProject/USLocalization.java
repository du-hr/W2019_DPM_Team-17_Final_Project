...
public class USLocalization {
  ...
  public static void fallingEdge() {
      double angleA, angleB, turningAngle; // Variables to use to get the 0ï¿½ angle
      //Get the distance from the US sensor
      usSensor.fetchSample(usData, 0);
      currentDist[0] = (int)(usData[0]*100);
      ...
      leftMotor.stop(true);
      rightMotor.stop();
      //Calculate the change in angle of the robot from the 0ï¿½ angle
      if (angleA < angleB) {
      ...
      //Calculate the angle by which to turn to face the 0ï¿½ angle
      turningAngle = deltaTheta + odometer.getXYT()[2];
      //Turn towards the 0ï¿½ angle
